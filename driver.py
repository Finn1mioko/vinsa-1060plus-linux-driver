...
It also supports the 10-button multimedia panel found on the tested device.
"""
...
# ---------------------------------------------------------------------------
#
# On the tested device, touching the multimedia strip produces a raw Y value
# far outside the normal tablet coordinate range. The observed values are
# approximately 61440..61640.
#
# The raw X coordinate identifies one of 10 equally spaced physical zones.

MEDIA_PANEL_Y_MIN = 60000

MEDIA_PANEL_X_CENTERS = (
    200,
    609,
    1018,
    1427,
...
# (display name, key sequence)
# A sequence containing multiple keys is emitted as a key chord.
MEDIA_PANEL_BUTTONS = (
    ("mute", (ecodes.KEY_MUTE,)),
    ("volume_down", (ecodes.KEY_VOLUMEDOWN,)),
    ("volume_up", (ecodes.KEY_VOLUMEUP,)),
    ("media", (ecodes.KEY_MEDIA,)),
    ("play_pause", (ecodes.KEY_PLAYPAUSE,)),
    ("previous_song", (ecodes.KEY_PREVIOUSSONG,)),
    ("next_song", (ecodes.KEY_NEXTSONG,)),
    ("homepage", (ecodes.KEY_HOMEPAGE,)),
    ("calculator", (ecodes.KEY_CALC,)),
    ("show_desktop", (ecodes.KEY_LEFTMETA, ecodes.KEY_D)),
)


# Only volume buttons repeat while held.
MEDIA_PANEL_REPEATABLE = {
    1,  # volume_down
    2,  # volume_up
}

MEDIA_PANEL_REPEAT_DELAY = 0.40
MEDIA_PANEL_REPEAT_INTERVAL = 0.15
...
class Config:
    ...
    pressure_in_min: int = 1560
    pressure_in_max: int = 1050

    pressure_out_min: int = 0
    pressure_out_max: int = 2047

    pressure_threshold_press: int = 60
    pressure_threshold_release: int = 30
...
class T501Parser:
    ...
    def parse(self, data: bytes | array):
        ...
        # Multimedia panel is identified from RAW coordinates.
        # Do this before axis swapping/inversion.
        media_panel = y0 >= MEDIA_PANEL_Y_MIN
        ...
        if self.raw:
            logging.debug(
                "RAW action=%d x=%d y=%d pressure_raw=%d "
                "pen_btn=%d key=%d media=%s bytes=%s",
                action,
                x,
                y,
                pressure_raw,
        ...
        # Media-panel data does not contain real pen-pressure information.
        # Therefore it must not alter the current pen touch state.
        if not media_panel:
            ...
            if self.touch != previous_touch:
                logging.info(
                    "TOUCH STATE %s -> %s "
                    "(action=%d raw_pressure=%d pressure=%d)",
                    previous_touch,
                    self.touch,
                    action,
                    pressure_raw,
...
class T501Driver:
    def __init__(
        ...
        # Current multimedia-panel zone.
        self.active_media_button: Optional[int] = None

        # Timestamp when the next repeat is allowed.
        self.media_repeat_deadline: Optional[float] = None

        self.running = True

        self.parser = T501Parser(
            cfg,
            permissive,
            raw,
        )
        ...
        self.HEARTBEAT_TIMEOUT_THRESHOLD = 3
    ...
    def find(self):
        ...
        if dev is None:
            raise RuntimeError(
                f"USB device "
                f"{self.cfg.vendor_id:04x}:{self.cfg.product_id:04x} "
                f"not found"
            )
    ...
    def prepare_device(self):
        ...
        # detach all three interfaces first.
        logging.info(
        ...
        for iface in (0, 1, 2):
            ...
            if active:
                ...
                try:
                    ...
                    logging.warning(
                        "detach interface %d failed: %r",
                        iface,
                        exc,
                    )

        # Match upstream:
        # set configuration after detaching.
        logging.info(
        ...
        # claim ONLY interface 2.
        logging.info(
            "Claiming USB interface %d",
            CONTROL_IFACE,
        )
        ...
        # The working driver reads the data endpoint from interface 1
        # but does not explicitly claim that interface.
        cfg = dev.get_active_configuration()

        data_iface = usb.util.find_descriptor(
            cfg,
        ...
        if data_iface is None:
            raise RuntimeError(
                "T501 data interface 1 not found"
            )
        ...
        if endpoint is None:
            raise RuntimeError(
                "T501 interrupt IN endpoint on interface 1 not found"
            )

        self.endpoint = endpoint

        logging.info(
            "data endpoint iface=%d addr=0x%02x "
            "max_packet=%d interval=%d",
            DATA_IFACE,
    ...
    def enter_full_area(self):
        ...
        for i, (w_value, report) in enumerate(
            T501_REPORTS,
            1,
        ):
            logging.info(
                "SET_REPORT #%d wValue=0x%04x data=%s",
                i,
                w_value,
                report.hex(" "),
            )
        ...
        logging.info(
            "Re-running prepare_device() after SET_REPORT sequence "
            "to latch full-area mode"
        )
    ...
    def create_uinput(self):
        pen_caps = {
            ...
            ecodes.EV_ABS: [
                ...
                (
                    ...
                    AbsInfo(
                        0,
                        self.cfg.pressure_out_min,
                        self.cfg.pressure_out_max,
                        0,
                        0,
                        self.cfg.resolution_pressure,
                    ),
                ),
            ],
        }
        ...
        logging.info(
            "uinput created: "
            "VINSA-1060-T501 + VINSA-1060-T501 Buttons"
        )

    def _button_press_events(
        self,
        name: str,
    ) -> list[tuple[int, int]]:
        if name == "e":
    ...
    def release_tablet_button(
        ...
        if self.active_tablet_button in (
            "wheel_up",
            "wheel_down",
        ):
            # Wheel buttons are one-shot:
            # there is no held EV_KEY to release.
            self.active_tablet_button = None
            return

        if self.active_tablet_button is not None:
            events = self._button_press_events(
                self.active_tablet_button
            )

            for ev_type, code in reversed(events):
                self.virtual_keys.write(
                    ev_type,
                    code,
                    0,
                )
    ...
    def handle_tablet_key(
        ...
        if name is None:
            logging.debug(
                "UNKNOWN tablet_key=0x%04x",
                tablet_key,
            )
            return

        # Holding a button produces many identical HID reports.
        # Only the transition into the new state should generate a key event.
        ...
        logging.debug(
            "TABLET BUTTON DOWN %s (key=0x%04x)",
            name,
            tablet_key,
        )

    # -----------------------------------------------------------------------
    # Multimedia panel handling
    # -----------------------------------------------------------------------

    def _media_button_press(
        self,
        index: int,
    ):
        if self.virtual_keys is None:
        ...
        name, keys = MEDIA_PANEL_BUTTONS[index]
        ...
        logging.info(
            "MEDIA PANEL BUTTON #%d %s (x_center=%d)",
            index + 1,
            name,
            MEDIA_PANEL_X_CENTERS[index],
        )

    def handle_media_panel(
        self,
        raw_x: int,
        raw_y: int,
    ) -> bool:
        # Normal tablet coordinate range means the media panel
        # is no longer active.
        if raw_y < MEDIA_PANEL_Y_MIN:
            ...
            self.media_repeat_deadline = None

            return False

        # Find the nearest of the ten known button centers.
        index = min(
            range(len(MEDIA_PANEL_X_CENTERS)),
            key=lambda i: abs(
                raw_x - MEDIA_PANEL_X_CENTERS[i]
            ),
        )

        now = time.monotonic()

        # New media-panel zone -> immediate one-shot.
        if index != self.active_media_button:
            self._media_button_press(index)

            self.active_media_button = index

            if index in MEDIA_PANEL_REPEATABLE:
                self.media_repeat_deadline = (
                    now + MEDIA_PANEL_REPEAT_DELAY
                )
            else:
                self.media_repeat_deadline = None
        ...
        # Same zone is still held.
        # Non-repeatable buttons do nothing until release.
        if index not in MEDIA_PANEL_REPEATABLE:
            return True

        # Volume buttons repeat after a short initial delay.
        if self.media_repeat_deadline is not None:
            if now >= self.media_repeat_deadline:
                self._media_button_press(index)

                self.media_repeat_deadline = (
                    now + MEDIA_PANEL_REPEAT_INTERVAL
                )
    ...
    def emit(self, p):
        ...
        # Multimedia panel reports use a special raw Y range.
        # Handle them before normal tablet-key / pen processing.
        if p["media_panel"]:
            # If a pen touch was active before entering the media panel,
            # release it once.
            if self.active_media_button is None:
                if self.parser.touch:
                    self.release_pen(
                        reason="media-panel"
                    )
        ...
        # Normal Y means the media panel is no longer active.
        self.handle_media_panel(
            p["raw_x"],
            p["raw_y"],
        )
        ...
        self.handle_tablet_key(
            p["tablet_key"]
        )

        # Normal mode mirrors upstream action filtering.
        # Diagnostic permissive mode processes unknown actions too.
        usable_action = (
            p["accepted"]
            or self.permissive
        )
        ...
        if not usable_action:
            if not p["touch"]:
                # Make sure an unknown release cannot leave
                # a stale button down.
                self.release_pen(
                    reason=f"unknown-action-{p['action']}"
                )
        ...
        if not p["valid_position"]:
            ...
            if not p["touch"]:
                self.release_pen(
                    reason="invalid-position-release"
                )

            return

        self.virtual_pen.write(
            ecodes.EV_KEY,
            ecodes.BTN_TOOL_PEN,
            1,
        )
        ...
        self.virtual_pen.write(
            ecodes.EV_KEY,
            ecodes.BTN_STYLUS,
            1 if p["pen_btn"] == 4 else 0,
        )

        self.virtual_pen.write(
            ecodes.EV_KEY,
            ecodes.BTN_STYLUS2,
            1 if p["pen_btn"] == 6 else 0,
        )

        self.virtual_pen.syn()

        logging.debug(
            "OUT x=%d y=%d pressure=%d "
            "touch=%s action=%d pen_btn=%d",
            p["x"],
            p["y"],
            p["pressure"],
            p["touch"],
    ...
    def release_pen(
        ...
        # Avoid a stuck modifier if the USB stream stops
        # while a tablet key is held.
        self.release_tablet_button(
            reason
        )
    ...
    def run(self):
        ...
        logging.info(
            "DRIVER READY -- waiting for 64-byte T501 reports"
        )
            ...
            try:
                ...
                if errno in (110,):
                    ...
                    if self.debug and (
                        ...
                        or self.timeout_count % 50 == 0
                    ):
                        logging.debug(
                            "USB read timeout #%d (read=%d)",
                            self.timeout_count,
                            self.read_count,
                        )
                    ...
                    if (
                        ...
                        self.release_pen(
                            reason="silence-timeout (pen lifted)"
                        )

                        logging.info(
                            "No data for %d consecutive timeouts "
                            "(~%dms) -- doing a full re-arm cycle "
                            "(reset+detach+claim+SET_REPORT)",
                            self.consecutive_timeouts,
                            self.consecutive_timeouts
                            * self.cfg.timeout_ms,
                        )
...
def main() -> int:
    ...
    ap.add_argument(
        "--raw",
        action="store_true",
        help="dump every 64-byte report",
    )

    ap.add_argument(
        "--permissive-actions",
        action="store_true",
        help="process unknown pen_action values",
    )
